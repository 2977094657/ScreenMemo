...
<details>
<summary>如何接收应用更新？</summary>

- App 进入主界面或从后台回到前台后会自动检查 GitHub Releases 上的最新稳定版本，并默认按 6 小时间隔防抖，避免频繁打扰。
- 检测到新版本时会弹出更新提示，支持下载、忽略此版本或关闭窗口；忽略只对当前版本生效，后续更高版本仍会提示。
- 也可以在“设置 → 关于 → 检查更新”手动检查；手动检查不受自动检测防抖限制。
- 点击下载后会在 App 内下载匹配设备架构的 APK，并交给 Android 系统安装器完成安装；首次使用可能需要允许“安装未知来源应用”。
</details>
...
<details>
<summary>如何查看 App 运行状态？</summary>

- 在“设置 → App 运行状态”中查看采集服务、权限、数据库、存储、OCR / AI 处理和后台任务的结构化健康状态
- 页面顶部和每个模块卡片都用状态条显示健康趋势；默认每格 1 分钟，点击或悬停单个状态条可查看该时间段的检查次数、成功数和失败数
- 右上角“时间线设置”图标会从底部弹出设置面板，面板内通过类似“生成回放”的滑块选择器切换最近 72 分钟、6 小时、24 小时、7 天、30 天，也可以输入自定义“最近 xx 小时”；每格粒度可滑动切换为 1 分钟、5 分钟、30 分钟、1 小时或 6 小时
- 健康检查会在应用启动后自动调度，并在回到前台时补跑；应用进程仍存活或被前台采集服务保活时，会按 1 分钟间隔持续检测
- Android 原生侧会通过 AlarmManager 每 1 分钟尽量唤醒并补写健康状态；若设备省电策略、精确闹钟权限或用户强行停止应用限制后台唤醒，页面会在权限模块中体现降级原因
- 健康状态使用 1 分钟基础桶长期写入数据库，不自动清理历史；关键异常写入健康事件表，不依赖可被清空的日志文件
</details>

<details>
<summary>如何导出故障排查日志？</summary>

- 在“设置 → 日志管理”中按 `output/logs` 文件夹层级浏览日志，逐层进入年 / 月 / 日 / 子目录
- 当前层级只展示直接子项，文件夹和文件都可以单独分享或删除，也可以通过右上角操作分享所有日志
- 分享时会打包成 ZIP，并保留 `output/logs/...` 路径，便于定位问题
</details>
