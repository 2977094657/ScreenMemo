...
void main() {
  ...
  test(
    'strict full transcript can exceed effective cap and fallback to trimmed',
    () async {
      ...
      try {
        ...
        final List<AIMessage> strict = await ChatContextService.instance
            .loadRawTranscriptForPrompt(cid: 'strict-cid', maxTokens: 0);
        expect(strict.length, 2);

        final int strictTokens = PromptBudget.approxTokensForMessagesJson(
          strict,
        );
        ...
        final int trimmedTokens = PromptBudget.approxTokensForMessagesJson(
          trimmed,
        );
