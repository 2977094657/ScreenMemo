...
import 'package:screen_memo/services/ai_request_gateway.dart';
...
void main() {
  test(
    'responses terminal events do not duplicate final content',
    () async {
      ...
      final Future<void> serverDone = () async {
        await for (final HttpRequest req in server) {
          ...
          req.response.headers.set(HttpHeaders.contentTypeHeader,
              'text/event-stream; charset=utf-8');
          req.response.headers.set(HttpHeaders.cacheControlHeader, 'no-cache');

          await Future<void>.delayed(const Duration(milliseconds: 20));

          await _writeSseEvent(req.response, 'response.output_text.delta', <String, dynamic>{
          ...
          await _writeSseEvent(req.response, 'response.output_text.done', <String, dynamic>{
          ...
          await _writeSseEvent(req.response, 'response.content_part.done', <String, dynamic>{
          ...
          await _writeSseEvent(req.response, 'response.output_item.done', <String, dynamic>{
          ...
          await _writeSseEvent(req.response, 'response.completed', <String, dynamic>{
