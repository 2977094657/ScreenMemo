...
class ModelIconUtils {
  ...
  static Future<void> _ensureManifestLoaded() async {
    ...
    try {
      ...
      final Map<String, dynamic> map = json.decode(manifestJson) as Map<String, dynamic>;
      final set = <String>{};
  ...
  static Iterable<String> _candidateTokens(String s) {
    ...
    final root = norm.split('-').firstWhere((e) => e.isNotEmpty, orElse: () => norm);
  ...
  static String getIconPath(String? modelName) {
    ...
    // Qwen / 通义 / Ali
    if (m.contains('qwen') || m.contains('dashscope') || m.contains('ali') || m.contains('通义')) {
      return 'assets/icons/ai/qwen-color.svg';
    }

    // Zhipu / GLM
    if (m.contains('glm') || m.contains('chatglm') || m.contains('智谱') || m.contains('zhipu')) {
    ...
    if (m.contains('spark') || m.contains('iflytek') || m.contains('讯飞') || m.contains('xinghuo')) {
    ...
    // GPT / OpenAI / o3 / o1
    if (m.contains('gpt') || m.contains('openai') || m.contains('o3') || m.contains('o1')) {
      return 'assets/icons/ai/openai.svg';
    }
