...
class AdvancedSearchQuery {
  ...
  static AdvancedSearchQuery? tryParse(Object? raw) {
    ...
    final List<String> must = _readStringList(m['must'])
        .map(_cleanTermText)
        .where((e) => e.isNotEmpty)
        .toList(growable: false);
    final List<String> any = _readStringList(m['any'])
        .map(_cleanTermText)
        .where((e) => e.isNotEmpty)
        .toList(growable: false);
    final List<String> mustNot = _readStringList(m['must_not'])
        .map(_cleanTermText)
        .where((e) => e.isNotEmpty)
        .toList(growable: false);

    final List<String> phrases = _readStringList(m['phrases'])
        .map(_cleanPhraseText)
        .where((e) => e.isNotEmpty)
        .toList(growable: false);
    final List<String> phrasesAny = _readStringList(m['phrases_any'])
        .map(_cleanPhraseText)
        .where((e) => e.isNotEmpty)
        .toList(growable: false);
  ...
  String toFtsMatch({
    ...
    for (final String term in must.take(maxGroups)) {
      final List<String> tokens =
          _termTokens(term, maxTokens: maxTokensPerGroup);
    ...
    if (anyGroups.isNotEmpty) {
      clauses.add(anyGroups.length == 1 ? anyGroups.single : '(${anyGroups.join(' OR ')})');
    }
    ...
    if (anyPhrases.isNotEmpty) {
      clauses.add(anyPhrases.length == 1 ? anyPhrases.single : '(${anyPhrases.join(' OR ')})');
    }

    if (clauses.isEmpty) return '';
    final String positive =
        (clauses.length == 1) ? clauses.single : '(${clauses.join(' AND ')})';
    ...
    final String negative =
        (notGroups.length == 1) ? notGroups.single : '(${notGroups.join(' OR ')})';
  ...
  AdvancedSearchLikeQuery toLikeSpec({
    ...
    return AdvancedSearchLikeQuery(
      ...
      mustPhrases: phrases.take(maxGroups).map(_cleanPhraseText).where((e) => e.isNotEmpty).toList(growable: false),
      anyPhrases: phrasesAny.take(maxGroups).map(_cleanPhraseText).where((e) => e.isNotEmpty).toList(growable: false),
    );
  }
}
