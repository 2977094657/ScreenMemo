...
class ImeExclusionService {
  ...
  static bool _isImeByRegex(String pkg) =>
      _imeRegexes.any((re) => re.hasMatch(pkg));
  ...
  static bool _shouldExclude(String pkg) =>
      _isImeByRegex(pkg) || _isAutomationAssist(pkg);
  ...
  static Future<List<Map<String, String>>> getEnabledImeList() async {
    try {
      ...
      return list
          .map(
            (e) =>
                Map<String, String>.from(Map<String, dynamic>.from(e as Map)),
          )
          .toList();
    } catch (_) {
      return const [];
  ...
  static Future<List<AppInfo>> computeExcludedImeFrom(
    List<AppInfo> input,
  ) async {
    final enabledImes = await getEnabledImePackages();
