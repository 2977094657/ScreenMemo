...
class TimelineJumpService {
  ...
  final ValueNotifier<TimelineJumpRequest?> requestNotifier =
      ValueNotifier<TimelineJumpRequest?>(null);
          ...
          if (req != null) {
            try {
              FlutterLogger.nativeDebug(
                'TimelineJump',
                'timelineShown 事件触发，重发跳转请求 路径=' + req.filePath,
              );
            } catch (_) {}
            // 通过赋同值触发监听方刷新处理
            requestNotifier.value = TimelineJumpRequest(
  ...
  Future<void> jumpToFilePath(String filePath) async {
    try {
      await FlutterLogger.info('JumpService：jumpToFilePath 路径=' + filePath);
    } catch (_) {}
    _ensureInitialized();
    // 尽可能回到根页面，展示底部主导航
    try {
