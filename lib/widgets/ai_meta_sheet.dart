...
class AiMetaSheet {
  static Future<_AiMetaPayload?> _loadMeta(
    ...
    try {
      ...
      final ocrFut =
          includeOcr ? ScreenshotDatabase.instance.getScreenshotByPath(p) : null;
      final row = await rowFut.catchError((_) => null);
      final rec = await ocrFut?.catchError((_) => null);
      final String ocrText = ((rec as dynamic)?.ocrText as String?)?.trim() ?? '';
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (ctx) {
        ...
        final List<String> fallbackTagsValue =
            fallbackTags?.map((e) => e.trim()).where((e) => e.isNotEmpty).toList() ??
            const <String>[];
        ...
        return DraggableScrollableSheet(
          ...
          expand: false,
          builder: (_, ctrl) {
            return FutureBuilder<_AiMetaPayload?>(
              ...
              builder: (c, snap) {
                ...
                final String desc = dbDesc.isNotEmpty ? dbDesc : fallbackDesc;
                final String range = dbRange.isNotEmpty
                    ? dbRange
                    : (fallbackRangeText.isNotEmpty ? fallbackRangeText : p);
